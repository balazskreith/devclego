...
void _fsm_fire(int32_t event, void *arg)
{
	...
	_str_state(this->actual, this->state_str);
	INFOPRINT("FSM: %s event is fired, actual state is: %s", this->event_str, this->state_str);

	switch(this->actual)
	{
	case FSM_STATE_RUN:
		this->actual = _fsm_run_trans(event, arg);
			break;
	case FSM_STATE_HALT:
		this->actual = _fsm_halt_trans(event, arg);
			break;
	default:
	...
	_str_state(this->actual, this->state_str);
	INFOPRINT("FSM: Transitions performed, the actual state is: %s (%d)", this->state_str, this->actual);
	signal_unlock(this->signal);
}
